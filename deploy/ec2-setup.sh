#!/bin/bash
set -euo pipefail

APP_DIR="$HOME/noteg"

echo "──────────────────────────────────────────────────"
echo " NoteG — EC2 host bootstrap"
echo "──────────────────────────────────────────────────"

install_docker_ubuntu() {
    echo "[+] Detected Ubuntu/Debian — installing Docker from the official repo..."
    sudo apt-get update
    sudo apt-get install -y ca-certificates curl gnupg
    sudo install -m 0755 -d /etc/apt/keyrings
    curl -fsSL https://download.docker.com/linux/ubuntu/gpg \
        | sudo gpg --dearmor -o /etc/apt/keyrings/docker.gpg
    sudo chmod a+r /etc/apt/keyrings/docker.gpg
    echo \
        "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/docker.gpg] https://download.docker.com/linux/ubuntu \
        $(. /etc/os-release && echo "$VERSION_CODENAME") stable" \
        | sudo tee /etc/apt/sources.list.d/docker.list > /dev/null
    sudo apt-get update
    sudo apt-get install -y docker-ce docker-ce-cli containerd.io docker-compose-plugin
}

if command -v docker >/dev/null 2>&1; then
    echo "[=] Docker already installed: $(docker --version)"
else
    if [ -f /etc/os-release ]; then
        . /etc/os-release
        case "$ID" in
            ubuntu|debian) install_docker_ubuntu ;;
            *) echo "Unsupported distro '$ID'. Install Docker manually." >&2; exit 1 ;;
        esac
    else
        echo "Cannot detect distro (no /etc/os-release)." >&2
        exit 1
    fi
fi

echo "[+] Enabling and starting the Docker daemon..."
sudo systemctl enable --now docker

echo "[+] Adding '$USER' to the docker group..."
sudo usermod -aG docker "$USER"

echo "[+] Creating app directory at $APP_DIR..."
mkdir -p "$APP_DIR"

echo ""
echo "✅ Host bootstrap complete."
echo "   docker:  $(sudo docker --version)"
echo "   compose: $(sudo docker compose version | head -n1)"
